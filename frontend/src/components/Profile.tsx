...
<div className="text-center"> <p className={`text-2xl font-bold text-${color}-400`}>{value}</p> <p className="text-sm text-gray-400">{label}</p> </div> );
const ContactLine: FC<{ Icon: FC<any>; text?: string }> = ({ Icon, text }) => (

<div className="flex items-center text-gray-400"> <Icon className="h-4 w-4 mr-3" /> <span className="text-sm">{text || '—'}</span> </div> );
