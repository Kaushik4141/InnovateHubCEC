...
import {
  ...
  Pin, Trash2, Smile, Image as ImageIcon, Video as VideoIcon
} from 'lucide-react';
...
const Chat: React.FC = () => {
  ...
  const messageDomId = (m: Msg, idx: number) => (m as any)?._id || (m as any)?.clientId || `idx-${idx}`;
  ...
  useEffect(() => {
    if (!activeId && contacts.length > 0) {
      setScope('dm');
      setActiveId(contacts[0].user._id);
    }
  }, [contacts, activeId]);
      ...
      <div className="h-[calc(100vh-64px)] sm:h-[calc(100vh-72px)] bg-gray-900 text-white flex overflow-hidden">
                ...
                {Array.from(onlineUsers).map(id => {
                  ...
                  return (
                    <li key={id} className="flex items-center gap-3 p-2 rounded-lg bg-gray-800/50">
                      ...
                      {contact ? (
                        <>
                          <img src={avatarUrlFrom(contact.user._id, contact.user.fullname, contact.user.avatar)} alt={contact.user.fullname} className="h-6 w-6 rounded-full object-cover" />
        ...
        <main className="flex-1 flex flex-col h-full overflow-hidden bg-gray-900">
          <header className="h-14 flex items-center px-4 border-b border-gray-800 bg-gray-900/95 backdrop-blur-sm">
            <div className="flex items-center justify-between w-full">
              <div className="flex items-center gap-2">
                ...
                <h1 className="text-lg font-semibold truncate max-w-[150px] sm:max-w-xs">{selectedConversation?.name || 'Select a chat'}</h1>
                {selectedConversation?.online && <span className="text-xs text-green-400 ml-2">Active now</span>}
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="relative mr-2">
                  <input
                    ...
                    className="w-40 sm:w-48 px-3 py-1.5 bg-gray-800 border border-gray-700 rounded-lg text-sm text-white focus:outline-none focus:ring-1 focus:ring-purple-500"
                  />
          ...
          <section ref={messagesRef} className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-900">
            ...
            {pinnedMessage && (
              <div className="p-3 bg-gray-800 border border-gray-700 rounded-lg flex items-center justify-between text-sm text-gray-300">
                <div className="flex items-center gap-2 cursor-pointer" onClick={() => jumpToMessage((pinnedMessage as any)._id)}>
                  <Pin className="h-4 w-4 text-purple-400" />
                  <div className="truncate">{pinnedMessage.type === 'text' ? pinnedMessage.content : `Media: ${pinnedMessage.type}`}</div>
                </div>
                <button onClick={() => { setPinnedMessage(null); setMessages(prev => prev.map(m => ((m as any)._id === (pinnedMessage as any)._id ? ({ ...m, pinned: false } as Msg) : m))); }} className="text-gray-400 hover:text-white">
                  <X className="h-4 w-4" />
                </button>
              </div>
            )}
            ...
            {messages.length > 0 && messages.map((m, idx) => {
              ...
              return (
                <div key={mid} data-mid={mid} className={`flex ${isOwn ? 'justify-end' : 'justify-start'} group`}>
                  <div className={`flex items-end space-x-2 max-w-[90%] sm:max-w-sm md:max-w-md lg:max-w-lg xl:max-w-xl ${isOwn ? 'flex-row-reverse space-x-reverse' : ''}`}>
                    {avatarUrl ? (
                      <img
                        src={avatarUrl}
                        alt="avatar"
                ...
                <div className={`px-4 py-2 rounded-2xl shadow transition-all duration-150 ${isOwn ? 'bg-purple-600 text-white rounded-br-md' : 'bg-gray-800 text-gray-300 rounded-bl-md'} hover:scale-[1.02] ${highlightId === mid ? 'ring-2 ring-purple-400' : ''}`}>
                  ...
                  {m.type === 'image' ? (
                    ...
                    <p className="text-sm whitespace-pre-wrap break-words max-w-full overflow-hidden">{m.content}</p>
                  )}

                  {/* Reactions */}
                  {m.reactions && Object.keys(m.reactions).some(e => (m.reactions![e] || []).length > 0) && (
                    <div className="flex items-center space-x-1 mt-2 flex-wrap">
                      {Object.entries(m.reactions).map(([emoji, users]) => users && users.length > 0 && (
                ...
                <div className="opacity-0 group-hover:opacity-100 transition text-gray-400 hover:text-white p-1 rounded-md hover:bg-gray-700 flex items-center space-x-1">
                  {/* Reaction picker */}
                  <div className="relative">
