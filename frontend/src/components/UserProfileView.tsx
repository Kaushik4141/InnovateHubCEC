                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {projects.length === 0 ? (
                      ...
                      projects.map((p: any, idx: number) => (
                        <div key={idx} className="bg-gray-800 rounded-xl shadow-md border border-gray-700 overflow-hidden">
                            ...
                            <div className="mb-4 grid grid-cols-1 md:grid-cols-2 gap-2">
                              {p.postFile.map((fileUrl: string, fileIdx: number) =>
                                fileUrl.match(/\.(mp4|webm|ogg)$/i) ? (
                                  ...
                                  <img key={fileIdx} src={fileUrl} alt={p.title} className="w-full max-h-64 object-cover rounded-lg border border-gray-700" />
                                )
                              )}
                            </div>
                          )}
                          <div className="p-4">
                            ...
                            {p.description && <ExpandableText text={p.description} maxLength={180} />}
